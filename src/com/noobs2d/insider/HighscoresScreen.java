...
public class HighscoresScreen extends DynamicScreen {
	...
	background = new DynamicSprite(new TextureRegion(Assets.mainMenu, 0, 0, 800, 480), Settings.WIDTH / 2, Settings.HEIGHT / 2);
	...
	label = new DynamicText(new BitmapFont(Gdx.files.internal("data/fonts/TRAJANPRO42.fnt"), new TextureRegion(Assets.trajanPro42), false), "HIGHSCORES");
	label.position.set(Settings.WIDTH / 2, 265);
	label.color.a = 0f;
	label.interpolateAlpha(1f, Linear.INOUT, 500, true);
	String highscores = "";
	for (int i = 0; i < 5; i++)
	    highscores += Settings.highscores[i] + "\n";
	scores = new DynamicText(new BitmapFont(Gdx.files.internal("data/fonts/TRAJANPRO35.fnt"), new TextureRegion(Assets.trajanPro35), false), highscores);
	scores.position.set(Settings.WIDTH / 2, 126);
	scores.color.a = 0f;
	scores.interpolateAlpha(1f, Linear.INOUT, 500, true);
    }
	...
	if (back.getBounds().contains(x, y)) {
	    ...
	    label.interpolateAlpha(0f, Linear.INOUT, 500, true);
	    scores.interpolateAlpha(0f, Linear.INOUT, 500, true);
